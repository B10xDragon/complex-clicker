...
export function click(s){const now=Date.now();s.stats.clicks++;s.combo=now-s.lastClick<2200*(s.upgrades.rhythm?1.35:1)?s.combo+1:1;s.lastClick=now;s.stats.bestCombo=Math.max(s.stats.bestCombo,s.combo);let amount=1+(s.upgrades.capacitors?.value||0);const crit=Math.random()<(0.05+(s.upgrades.precision?.value||0)*.05);if(crit)amount*=2+(s.upgrades.precision?.value||0);amount*=1+Math.min(s.combo,50)*.01;s.resources.energy+=amount;s.resources.credits+=(crit?.2:.1)+Math.min(s.combo,50)*.01;s.resources.energy=Math.min(s.resources.energy,1e300);s.stats.totalEnergy+=amount;s.stats.totalProduced+=amount;if(s.stats.clicks===1)recordProgress(s,'First click','The first spark entered the core.');return {amount,crit}}
