...
export function missions(s){
 ...
 {id:'daily',name:'Daily pulse',desc:'Generate 1,000 energy today (resets at 00:00 UTC)',goal:1000,progress:Math.max(0,s.stats.totalEnergy-s.dailyStart),reward:500},
...
export function discover(s,id){
 const costs={vega:1000,orion:100000,void:10000000};const cost=costs[id];
 if(!cost||!(s.research.exploration||s.upgrades.explorer)||s.exploration.regions[id]||s.resources.energy<cost)return false;
 s.resources.energy-=cost;s.exploration.regions[id]=1;return true;
}
