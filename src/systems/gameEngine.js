...
export function tick(s,dt){const prod=totalProd(s);for(const [r,v] of Object.entries(prod))s.resources[r]=(s.resources[r]||0)+v*dt;s.stats.time+=dt;s.stats.totalProduced+=Object.values(prod).reduce((a,b)=>a+b,0)*dt;if(s.boost>0)s.boost=Math.max(0,s.boost-dt);return prod}
export function buy(s,id,qty){const b=buildings.find(x=>x.id===id),n=s.buildings[id]||0;let bought=0;for(let i=0;i<qty;i++){const c=cost(b,n+bought);if(s.resources.credits<c)break;s.resources.credits-=c;bought++}s.buildings[id]+=bought;return bought}
export function buyUpgrade(s,id){const u=upgrades.find(x=>x.id===id);if(s.upgrades[id])return false;for(const [r,c] of Object.entries(u.cost))if((s.resources[r]||0)<c)return false;for(const [r,c] of Object.entries(u.cost))s.resources[r]-=c;s.upgrades[id]=u;return true}
