...
export function click(s){const now=Date.now();s.stats.clicks++;s.combo=now-s.lastClick<2200*(s.upgrades.rhythm?1.35:1)?s.combo+1:1;s.lastClick=now;s.stats.bestCombo=Math.max(s.stats.bestCombo,s.combo);const upgradeClickPower=Object.values(s.upgrades).filter(u=>u?.effect==='clickFlat').reduce((sum,u)=>sum+Math.max(0,Number(u.value)||0),0),buildingClickPower=(s.buildings?.clicking||0);let amount=1+upgradeClickPower+buildingClickPower;const crit=Math.random()<(0.05+(s.upgrades.precision?.value||0)*.05);if(crit)amount*=2+(s.upgrades.precision?.value||0);amount*=1+Math.min(s.combo,50)*.01;s.resources.energy+=amount;s.resources.credits+=(crit?.2:.1)+Math.min(s.combo,50)*.01;s.resources.energy=Math.min(s.resources.energy,1e300);s.stats.totalEnergy+=amount;s.stats.totalProduced+=amount;if(s.stats.clicks===1)recordProgress(s,'First click','The first spark entered the core.');return {amount,crit}}
