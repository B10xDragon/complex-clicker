...
import {ensureCivilization,migrateCivilization,civilizationTick,civilizationBonuses,eventTick} from './civilization.js';
import {ensureProgression,recordProgress,updateEra,buildingEffect,buildingMilestone,synergyMultiplier,prestigeBenefits,directiveProgress,civilizationDirectives} from './progression.js';
export {discover} from './exploration.js';
export const VERSION=9;
export function fresh(){const s={version:VERSION,resources:{energy:25,credits:15,research:0,data:0,matter:0,antimatter:0,darkEnergy:0,quantumCores:0,shards:0,knowledge:0},buildings:Object.fromEntries(buildings.map(b=>[b.id,0])),upgrades:{},research:{},stats:{clicks:0,totalEnergy:0,totalProduced:0,prestiges:0,ascensions:0,time:0,offline:0,bestCombo:0,longestSession:0},claimed:{},dailyDate:'',dailyStart:0,boostCooldown:0,combo:0,lastClick:0,boost:0,missions:{daily:0,tutorial:0},exploration:{unlocked:false,regions:{home:1},outposts:{},skills:{},frontier:0,missions:[]},cosmic:null,civilization:null,progression:null,log:['SYSTEM ONLINE — welcome, architect.'],settings:{reducedMotion:false,particles:true}};ensureCosmic(s);migrateCivilization(s);ensureProgression(s);return s}
...
export function normalize(p){
 ...
 if(p.civilization&&typeof p.civilization==='object')n.civilization=JSON.parse(JSON.stringify(p.civilization));
 if(p.progression&&typeof p.progression==='object')n.progression=JSON.parse(JSON.stringify(p.progression));
 migrateCivilization(n);ensureCosmic(n);
 ensureProgression(n);
...
export function totalProd(s){const skill=skillBonus(s),galaxy=galaxyBoosts(s),cosmic=cosmicBonuses(s),civ=civilizationBonuses(s),prestige=prestigeBenefits(s);let mult=1+(s.upgrades.reactors?.value||0)+(s.research.industrial?.value||0)+(s.research.efficiency?.value||0)+(s.upgrades.singularity?.value||0)+prestige.production;mult*=1+Math.floor(s.buildings.fusion/10)*.05;mult*=1+Math.sqrt(s.resources.shards)*.1+s.resources.knowledge*.5;mult*=1+(Object.keys(s.exploration.regions).length-1)*.05;mult*=1+skill.production+skill.frontier+galaxy.production+cosmic.production+(civ.industry||0);if(s.boost>0)mult*=2;const out={};for(const b of buildings){const level=s.buildings[b.id]||0,effect=buildingEffect(s,b.id),local=(1+(effect.production||0))*synergyMultiplier(s,b.id);for(const [r,v] of Object.entries(b.prod))out[r]=(out[r]||0)+v*level*local*(1+(effect[r]||0))}for(const [r,v] of Object.entries(outpostProduction(s)))out[r]=(out[r]||0)+v;if(s.research.ai)out.data=(out.data||0)*1.5;out.research=(out.research||0)*(1+(cosmic.research||0)+(civ.research||0));out.energy=(out.energy||0)*Math.max(.2,1-(civ.energyDemand||0));for(const r of Object.keys(out))out[r]*=mult;for(const r of ['energy','credits','research','knowledge'])if(out[r])out[r]*=1+(galaxy[r]||0);return out}
export function click(s){const now=Date.now();s.stats.clicks++;s.combo=now-s.lastClick<2200*(s.upgrades.rhythm?1.35:1)?s.combo+1:1;s.lastClick=now;s.stats.bestCombo=Math.max(s.stats.bestCombo,s.combo);const clickFlat=Object.values(s.upgrades).filter(u=>u?.effect==='clickFlat').reduce((sum,u)=>sum+Math.max(0,Number(u.value)||0),0);let amount=1+clickFlat;const crit=Math.random()<(0.05+(s.upgrades.precision?.value||0)*.05);if(crit)amount*=2+(s.upgrades.precision?.value||0);amount*=1+Math.min(s.combo,50)*.01;s.resources.energy+=amount;s.resources.credits+=(crit?.2:.1)+Math.min(s.combo,50)*.01;s.resources.energy=Math.min(s.resources.energy,1e300);s.stats.totalEnergy+=amount;s.stats.totalProduced+=amount;if(s.stats.clicks===1)recordProgress(s,'First click','The first spark entered the core.');return {amount,crit}}
export function tick(s,dt){if(!Number.isFinite(dt)||dt<=0)return totalProd(s);migrateCivilization(s);ensureProgression(s);const prod=totalProd(s);s.stats.totalEnergy+=(prod.energy||0)*dt;for(const [r,v] of Object.entries(prod))s.resources[r]=Math.min(1e100,(s.resources[r]||0)+v*dt);s.stats.time+=dt;s.boostCooldown=Math.max(0,s.boostCooldown-dt);if(Date.now()-s.lastClick>2200*(s.upgrades.rhythm?1.35:1))s.combo=0;s.stats.totalProduced+=Object.values(prod).reduce((a,b)=>a+b,0)*dt;if(s.boost>0)s.boost=Math.max(0,s.boost-dt);civilizationTick(s,dt);updateEra(s);if(Math.random()<dt*.012/60)eventTick(s);return prod}
export function buy(s,id,qty){const b=buildings.find(x=>x.id===id),n=s.buildings[id]||0;if(!b||Math.max(s.stats.totalEnergy,s.resources.energy)<b.unlock||!Number.isFinite(qty)||qty<=0)return 0;qty=Math.min(10000,Math.floor(qty));let bought=0;for(let i=0;i<qty;i++){const c=cost(b,n+bought);if(s.resources.credits<c)break;s.resources.credits-=c;bought++}s.buildings[id]+=bought;if(bought){recordProgress(s,n===0?'First automation online':`${b.name} purchased`,`Built ${bought} ${b.name}${bought===1?'':'s'}.`);const milestone=buildingMilestone(s,id);if(milestone&&!s.progression.milestones[`building-${id}-${milestone.level}`]){s.progression.milestones[`building-${id}-${milestone.level}`]=true;s.log.unshift(`${b.name.toUpperCase()} MILESTONE — ${milestone.label}`);}}return bought}
...
export function prestige(s){const gain=Math.floor(Math.sqrt(Math.max(0,s.resources.energy)/1e6));if(!gain)return 0;const keep={shards:s.resources.shards+gain,knowledge:s.resources.knowledge};const retained={stats:{...s.stats},research:s.research,upgrades:s.upgrades,claimed:s.claimed,dailyDate:s.dailyDate,dailyStart:s.dailyStart,settings:s.settings,progression:s.progression,exploration:{...fresh().exploration,skills:s.exploration.skills||{},frontier:s.exploration.frontier||0}};const n=fresh();Object.assign(s,n,retained);s.resources={...n.resources,...keep};s.stats.prestiges++;const benefits=prestigeBenefits(s);s.resources.credits+=benefits.credits;s.buildings.manual+=benefits.manual;s.buildings.solar+=benefits.solar;s.log.unshift(`TRANSCENDENCE COMPLETE +${gain} prestige shards`);recordProgress(s,'Prestige '+s.stats.prestiges,'A faster empire begins.');return gain}
export function ascend(s){if(!s.research.galactic||s.resources.shards<100)return false;const wasFirst=s.stats.ascensions===0,knowledge=s.resources.knowledge+1,stats={...s.stats,ascensions:s.stats.ascensions+1},claimed=s.claimed,settings=s.settings,skills=s.exploration.skills||{},frontier=s.exploration.frontier||0,cosmic=s.cosmic,progression=s.progression;Object.assign(s,fresh(),{stats,claimed,settings,progression});s.exploration.skills=skills;s.exploration.frontier=frontier;s.cosmic={...s.cosmic,...cosmic,unlocked:true,ascensionMilestones:stats.ascensions};s.resources.knowledge=knowledge;migrateCivilization(s);s.civilization.unlocked=true;s.log.unshift(wasFirst?'FIRST ASCENSION COMPLETE — choose one Cosmic Knowledge awakening':'ASCENSION COMPLETE — cosmic knowledge acquired');recordProgress(s,wasFirst?'First Ascension':'Ascension '+stats.ascensions,'The scale of Starforge has changed.');return true}
