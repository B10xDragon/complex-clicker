...
import {eras,buildingMilestones,prestigeMilestones,civilizationDirectives,opportunities,balance} from '../data/progressionData.js';
...
export function currentEra(s){
 const colonies=Object.keys(s.civilization?.colonies||{}).length;
 const galaxies=Object.keys(s.exploration?.regions||{}).length;
 const trade=s.civilization?.tradeRoutes?.length||0;
 const mega=Object.values(s.civilization?.megastructures||{}).reduce((a,b)=>a+b,0);
 let era=eras[0];
 for(const candidate of eras){
  const ok=(candidate.minEnergy===undefined||(s.stats.totalEnergy||0)>=candidate.minEnergy)
   ...
   &&(candidate.minColonies===undefined||colonies>=candidate.minColonies)
   &&(candidate.minTrade===undefined||trade>=candidate.minTrade)
...
export function directiveProgress(s,metric){const c=s.civilization||{};return ({population:c.population||0,specialized:Object.values(c.colonies||{}).filter(x=>x.specialization&&x.specialization!=='industrial').length,government:c.government?1:0,relations:Object.values(c.relations||{}).filter(x=>x>0).length,trade:c.tradeRoutes?.length||0,fleet:Object.values(c.fleets||{}).reduce((a,b)=>a+b,0),mega:Object.values(c.megastructures||{}).reduce((a,b)=>a+b,0)})[metric]||0;}
...
export {eras,buildingMilestones,prestigeMilestones,civilizationDirectives,opportunities};
