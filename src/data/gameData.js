...
upgrades.push(
 {id:'core-relay-network',cat:'Infrastructure',name:'Core Relay Network',cost:{energy:500,credits:100},desc:'+3 energy per click by routing building power back into the reactor. Requires High-density Capacitors.',effect:'clickFlat',value:3,req:['capacitors']},
 ...
 {id:'quantum-tap',cat:'Clicking',name:'Quantum Tap',cost:{energy:150000,research:8000,data:2500,quantumCores:1},desc:'+75 energy per click by collapsing a prepared probability field. Requires Stellar Pulse.',effect:'clickFlat',value:75,req:['stellar-pulse']},
 {id:'infinite-loop',cat:'Clicking',name:'Infinite Loop',cost:{energy:2500000,research:75000,data:25000,quantumCores:8},desc:'+250 energy per click by routing the reactor output through a recursive loop. Requires Quantum Tap.',effect:'clickFlat',value:250,req:['quantum-tap']},
 {id:'singularity-touch',cat:'Clicking',name:'Singularity Touch',cost:{energy:50000000,research:500000,data:150000,quantumCores:60,darkEnergy:25},desc:'+1,000 energy per click. Requires Infinite Loop.',effect:'clickFlat',value:1000,req:['infinite-loop']}
);
...
const starterParents=['home','home','home','vega','vega','vega','sirius','sirius','orion','orion','orion','void'];
const generatedGalaxies=Array.from({length:250-regions.length},(_,i)=>{const number=i+1,type=galaxyTypes[i%galaxyTypes.length],tier=Math.floor(i/12),angle=(i*137.508)*Math.PI/180,parent=i<starterParents.length?starterParents[i]:`g${String(Math.floor((i-starterParents.length)/3)+1).padStart(3,'0')}`,branch=i%12,baseCost=1000*Math.pow(1.22,i),cost=Math.ceil(baseCost*(1+branch*.14));return {id:`g${String(number).padStart(3,'0')}`,name:`${type.name} ${String(number).padStart(3,'0')}`,parent,galaxy:tier<8?'triangulum':tier<16?'perseus':'deepfield',type:type.id,cost,prod:type.prod,mapX:1200+Math.cos(angle)*(180+tier*82),mapY:1200+Math.sin(angle)*(180+tier*82),desc:type.desc};});
